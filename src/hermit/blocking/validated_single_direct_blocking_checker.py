"""Validated single direct blocking checker.

Faithful port of ``org.semanticweb.HermiT.blocking.ValidatedSingleDirectBlockingChecker``
from the Java HermiT OWL reasoner.

Classes
-------
ValidatedSingleDirectBlockingChecker
    Single blocking checker that tracks changes for validation purposes.
ValidatedBlockingObject
    Protocol for blocking objects that support validation.
ValidatedSingleBlockingObject
    Concrete blocking object for the validated single checker.
"""
...
class ValidatedBlockingObject(ABC):
    """Protocol for blocking objects that support validation.

    Both validated single and validated pairwise checkers use objects that
...
class ValidatedSingleDirectBlockingChecker(DirectBlockingChecker):
    """Single direct blocking checker with validation tracking.

    Tracks separate change flags for blocking and validation purposes, and
    maintains both core (blocking-relevant) and full labels.
    """
    ...
    def is_blocked_by(self, blocker: Node, blocked: Node) -> bool:
        ...
        assert isinstance(blocker_obj, ValidatedSingleBlockingObject)
        assert isinstance(blocked_obj, ValidatedSingleBlockingObject)
    ...
    def blocking_hash_code(self, node: Node) -> int:
        ...
        assert isinstance(obj, ValidatedSingleBlockingObject)
        return obj.m_blocking_relevant_hash_code
    ...
    def node_initialized(self, node: Node) -> None:
        if node.get_blocking_object() is None:
            node.set_blocking_object(ValidatedSingleBlockingObject(self, node))
        obj = node.get_blocking_object()
        assert isinstance(obj, ValidatedSingleBlockingObject)
        obj.initialize()
    ...
    def get_blocking_signature_for(self, node: Node) -> BlockingSignature:
        return ValidatedSingleBlockingSignature(self, node)
...
class ValidatedSingleBlockingObject(ValidatedBlockingObject):
    """Blocking object for the validated single direct blocking checker."""
...
class ValidatedSingleBlockingSignature(BlockingSignature):
    """Blocking signature for validated single direct blocking."""
    ...
    def __eq__(self, other: object) -> bool:
        if self is other:
            return True
        if not isinstance(other, ValidatedSingleBlockingSignature):
            return False
        return (
