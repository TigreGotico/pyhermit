...
This is a from-scratch course in OWL 2 DL reasoning and a tour of how pyhermit (a Python port of the Java [HermiT](https://github.com/phillord/hermit-reasoner) reasoner) implements it. It assumes you know basic logic, sets, and Python, but no description logic or tableau reasoning. Every term is defined on first use.

Read the chapters in order. Each chapter builds on the last and traces small worked examples step by step. Each chapter also cross-references the source files in `src/hermit/` so you can read code alongside theory.
...
- [`FAITHFULNESS_AUDIT.md`](../FAITHFULNESS_AUDIT.md): where and why pyhermit diverges from Java HermiT, with the soundness, completeness, and performance consequence of each divergence.
...
- Motik, Shearer, Horrocks, *Hypertableau Reasoning for Description Logics*, Journal of Artificial Intelligence Research 36 (2009), 165-228. This is the hypertableau paper. It defines the calculus that pyhermit's `Tableau` implements.
- W3C, *OWL 2 Web Ontology Language: Structural Specification and Functional-Style Syntax* (2nd ed., 2012). It defines the language pyhermit reads.
